...
def latest_monday_func(latest_date):
    today = latest_date
    offset = (today.weekday()) % 7
    last_moday = today - timedelta(days=offset)
    last_monday_string = datetime.strftime(last_moday,'%Y-%m-%d')
#    print(last_tuesday_string)
    return last_monday_string


#latest_monday_func(datetime.now())